...
|----------|-----------|-------------|---------|
| 批次轉化本週通話／會議錄音稿 | ❌ | ✅ | 批次掃描多檔、自動命名、同步更新彙整索引 |
| 批次歸檔本月收據與發票 | ❌ | ✅ | 掃描資料夾、重新命名、移動檔案 |
| 多平台收入 → 月度檢討簡報 | ❌ | ✅ | 同時讀取多份本地檔案、交叉比對後生成 |
| 掃描未回覆的報價單並批次催收 | ❌ | ✅ | 感知資料夾狀態、判斷哪些待處理、批次輸出 |
| 增量更新個人筆記索引 | ❌ | ✅ | 比對時間戳、只補差異、追加更新現有檔案 |
