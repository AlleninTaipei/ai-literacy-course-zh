...
| 專案（只限當前） | `<專案根目錄>\.claude\commands\` | `.claude/commands/` |
...
| 工作流程範例 | 批次掃描自動命名 / 歸檔搬移檔案 / 跨檔比對生成 / 感知狀態批次輸出 / 增量比對追加更新; 真實需求: 多份 Excel 彙整成一份 |
| 核心工作模式 | Spec-first / Atomic / Context / Scaffold / Review - 任務性質、技術複雜度、開發動機都不同, 但驅動每一次有效協作的, 始終是同一套溝通節奏 |
| AI 時代的數位資產 | AI-Assisted Development（AI 是 Developer）vs AI-Native Assets（AI 是 Agent）; 前者交付可執行工具, 後者交付 Workflow / Agent |
