...
每一次推論都是獨立事件, 模型不會自己保留上一句話的記憶. 你感覺對話「連得起來」, 是因為每次送出新訊息時, 應用程式都把之前的對話紀錄整段重新送給模型.

這份重新送出的紀錄, 加上你這次的輸入, 合稱上下文 (Context). 模型一次能處理的 Token 數量有上限, 稱為上下文視窗 (Context Window).

對話越長, 每次重送的紀錄就越長, 越早消耗這個上限. 一旦超過, 較早的內容就會被截斷或摘要.

> 開始一段長工作前, 先在對話開頭貼上「背景摘要」, 減少對早期內容的依賴.
> 部分平台提供的記憶功能 (例如 ChatGPT 的 Memory) , 其實是把重點另存成一份摘要, 在新對話開始時自動附加進上下文, 模型本身仍然無狀態.
...
| 使用 | 無狀態 + 多輪對話 | 模型不記事, 靠應用程式重送歷史紀錄撐起對話 |
