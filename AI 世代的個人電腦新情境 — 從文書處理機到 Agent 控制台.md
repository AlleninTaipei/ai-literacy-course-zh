...
#### 建立 GitHub 帳號與第一個 Repo

GitHub 是目前最主流的雲端 Git 平台。你可以把它想成「Git 帳本的雲端保險箱」——本機的 repo 住在你的電腦裡，GitHub 上的 repo 是它在網路上的備份與鏡像。

| 步驟 | 操作 |
|------|------|
| 1. 建立帳號 | 前往 [github.com](https://github.com)，點選「Sign up」，填入 Email、密碼、使用者名稱，完成驗證 |
| 2. 建立新 Repo | 登入後點選右上角「+」→「New repository」|
| 3. 填入 Repo 設定 | Repository name 填入你的專案名稱，選「Private」（私人），點選「Create repository」|
| 4. 複製 Repo URL | 頁面建立完成後，複製畫面上顯示的 HTTPS URL，格式如：`https://github.com/你的帳號/repo名稱.git` |

> 選「Private」表示只有你能看到這個 repo。之後如果需要分享給同事，可以在 Settings → Collaborators 邀請對方。
